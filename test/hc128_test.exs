defmodule HC128Test do
  ...
  test "decode_stream" do
    ...
    :ok = HC128.set_key_iv(c, <<66, 91, 41, 253, 183, 83, 197, 5, 131, 119, 232, 10, 80, 23, 128, 117>>, <<222, 173, 69, 193, 42, 200, 147, 206, 170, 0, 191, 182, 123, 64, 25, 167>>)
    ...
    assert o2 == <<224, 128, 255, 37, 110, 129, 169, 229, 81, 19, 221, 72, 48, 38, 12, 19>>
  end

  test "decode_bytes" do
    {:ok, c} = HC128.alloc_context()
    :ok = HC128.set_key_iv(c, <<66, 91, 41, 253, 183, 83, 197, 5, 131, 119, 232, 10, 80, 23, 128, 117>>, <<222, 173, 69, 193, 42, 200, 147, 206, 170, 0, 191, 182, 123, 64, 25, 167>>)
    {:ok, o1} = HC128.decode_stream c, "A"
    {:ok, o2} = HC128.decode_stream c, "B"
    assert o1 == <<137>>
    assert o2 == "2"
  end
end
